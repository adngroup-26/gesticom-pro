    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:600, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      <div style={{ background:'#fff', borderRadius:16, padding:28, width:360, maxWidth:'100%', boxShadow:'0 24px 60px rgba(0,0,0,0.2)' }}>
        ...
        <div style={{ display:'flex', gap:10 }}>
          <button onClick={onCancel} style={{ flex:1, padding:11, borderRadius:10, border:`1px solid ${C.g200}`, background:'#fff', cursor:'pointer', fontWeight:600, fontSize:14, color:C.g600 }}>Annuler</button>
          <button onClick={onConfirm} style={{ flex:1, padding:11, borderRadius:10, border:'none', background:C.err, color:'#fff', cursor:'pointer', fontWeight:700, fontSize:14 }}>🗑️ Supprimer</button>
        </div>
      </div>
    </div>
  )
...
export default function Clients({ eid, showToast, isAdm }) {
  ...
  async function createClient() {
    ...
    const { error } = await sb.from('clients').insert({
      entreprise_id: eid,
      nom: form.nom.trim(), telephone: form.tel.trim(),
      adresse: form.adresse.trim(), email: form.email.trim(),
      total_achats: 0,
    })
    if (error) showToast(error.message, 'error')
  ...
  async function updateClient() {
    ...
    const { error } = await sb.from('clients')
      .update({ nom: form.nom.trim(), telephone: form.tel.trim(), adresse: form.adresse.trim(), email: form.email.trim() })
      .eq('id', editClient.id).eq('entreprise_id', eid)
    if (error) showToast(error.message, 'error')
  ...
  return (
    <div style={{ padding: 24 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20, gap:12 }}>
        ...
        <button onClick={() => { setForm(EMPTY_CLIENT); setShowAdd(true) }}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:16 }}>
          {filt.map(c => (
            <div key={c.id} style={{ background:'#fff', borderRadius:12, border:`1px solid ${C.g200}`, padding:20, position:'relative' }}>
              ...
              <div onClick={() => loadHistorique(c)} style={{ cursor:'pointer' }}>
