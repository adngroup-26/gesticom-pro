  ...
  async function valider() {
    ...
    try {
      ...
      await sb.from('lignes_vente').insert(
        panier.map(x => ({ vente_id:vente.id, produit_id:x.id, produit_nom:x.nom, quantite:x.qty, prix_unitaire:x.prix_vente, prix_revient_unitaire:x.prix_revient, sous_total:x.prix_vente*x.qty }))
      )
      await Promise.all(panier.map(x => sb.from('produits').update({ stock: x.stock - x.qty }).eq('id', x.id)))

      setFacture({ ...vente, panier: panier.slice(), entreprise_nom: profil?.entreprises?.nom })
      setPanier([]); setClv('Client direct')
    ...
    <div style={{ padding:24, display:'grid', gridTemplateColumns:'1fr 330px', gap:20 }}>
      ...
      <div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(170px,1fr))', gap:12 }}>
          {filt.map(p => (
            <div key={p.id} onClick={() => addCart(p)} style={{ background:'#fff', borderRadius:12, border:`1px solid ${C.g200}`, padding:14, cursor:p.stock>0?'pointer':'default', opacity:p.stock>0?1:0.55 }}>
              ...
              <div style={{ marginTop:8, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                ...
                <span style={{ background:p.stock>0?C.priL:C.g100, color:p.stock>0?C.pri:C.g500, borderRadius:20, padding:'3px 9px', fontSize:12, fontWeight:600 }}>{p.stock>0?'+ Ajouter':'Épuisé'}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ background:'#fff', borderRadius:12, border:`1px solid ${C.g200}`, padding:20, position:'sticky', top:0, height:'fit-content', maxHeight:'calc(100vh - 40px)', overflowY:'auto' }}>
        ...
        <div style={{ marginBottom:12 }}>
          ...
          <select value={clv} onChange={e => setClv(e.target.value)} style={{ width:'100%', marginTop:4, padding:'8px 10px', borderRadius:8, border:`1px solid ${C.g200}`, fontSize:14 }}>
            <option>Client direct</option>
            {clis.map(c => <option key={c.id}>{c.nom}</option>)}
          </select>
        </div>

        {panier.length === 0 ? <div style={{ textAlign:'center', color:C.g500, padding:'30px 0', fontSize:14 }}>Panier vide</div> :
        <>
          {panier.map(item => (
